...
reference. The supported import paths are only `kasane` and
`kasane/standard-schema`; signatures in the installed declarations remain the
source of truth.
...
The `kasane/standard-schema` subpath exports runtime
...
Do not import `kasane/dist/*`, `kasane/internal/*`, or source-tree paths. Those
